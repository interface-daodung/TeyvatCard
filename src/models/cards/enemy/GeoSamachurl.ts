...
export default class GeoSamachurl extends Enemy {
    ...
    onCompleteMoveSama(): void {
        ...
        // Kích hoạt hiệu ứng rồi reset token để có thể tích lại sau.
        this.samaCount = -1;
        this.refreshSamaTokens();
    ...
    /** Buff: tất cả `Enemy` có `clan === 'hilichurl'` nhận thêm HP. */
    private buffHilichurlEnemies(): void {
        const allCards = this.scene.gameManager?.cardManager?.getAllCards?.() ?? [];
        const addHealth = Math.max(1, Math.floor(this.health / 3)); // 1/3, làm tròn xuống, min = 1
